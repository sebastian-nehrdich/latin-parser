...
from utils import load_word_embeddings,load_bert_embeddings
...
def read_arguments():
    ...
    if args_dict['task'] == 'number_of_children':
        ...
        args_dict['data_paths'] = write_extra_labels.add_language_model(args_dict['model_path'], args_dict['parser_path'], args_dict['domain'], args_dict['domain'],
                                                            use_unlabeled_data=args_dict['use_unlabeled_data'],
                                                            use_labeled_data=args_dict['use_labeled_data'])
    print("ARGS DIC KEYS",args_dict['data_paths'])
    ...
    for split in args_dict['splits']:
        if '_' in args_dict['domain']:
            alphabet_data_paths[split] = data_path + split + '_' + args_dict['domain'].split('_')[0]
        else:
    ...
    if args_dict['use_bert']:
        args_dict['bert_dict'], args_dict['bert_dim'] = load_bert_embeddings.load_bert_dict(use_aug=args.use_aug)
...
def build_model_and_optimizer(args):
    ...
    pos_type_table = construct_embedding_table(args.alphabets['pos_type_alphabet'], args.pos_dict, args.pos_dim, token_type='pos_type')
    case_table = construct_embedding_table(args.alphabets['case_alphabet'], args.pos_dict, args.pos_dim, token_type='case')
    number_table = construct_embedding_table(args.alphabets['number_alphabet'], args.pos_dict, args.pos_dim, token_type='number')    
    gender_table = construct_embedding_table(args.alphabets['gender_alphabet'], args.pos_dict, args.pos_dim, token_type='gender')
    verb_form_table = construct_embedding_table(args.alphabets['verb_form_alphabet'], args.pos_dict, args.pos_dim, token_type='verb_form')
    full_pos_table = construct_embedding_table(args.alphabets['full_pos_alphabet'], args.pos_dict, args.pos_dim, token_type='full_pos')        
    class_table = construct_embedding_table(args.alphabets['class_alphabet'], args.pos_dict, args.pos_dim, token_type='class')
    punc_table = construct_embedding_table(args.alphabets['punc_alphabet'], args.pos_dict, args.pos_dim, token_type='punc')    
    bert_table = construct_embedding_table(args.alphabets['wordpos_id_alphabet'], args.bert_dict, args.bert_dim, token_type='wordpos_id')


    model = Sequence_Tagger(args.word_dim, args.num_word, args.char_dim, args.num_char, args.use_pos_type, args.use_case, args.use_number,args.use_gender,args.use_verb_form,args.use_full_pos, args.use_char, args.use_class, args.use_punc, args.pos_dim, args.num_pos_type, args.num_case, args.num_number, args.num_gender, args.num_verb_form, args.num_full_pos, args.num_class, args.num_punc,
                            args.num_filters,
                            args.kernel_size, args.rnn_mode, args.hidden_size, args.num_layers, args.tag_space, args.num_auto_label,
                            embedd_word=word_table, embedd_char=char_table, embedd_pos_type=pos_type_table, embedd_case=case_table, embedd_gender=gender_table,embedd_number=number_table,embedd_verb_form=verb_form_table,embedd_full_pos=full_pos_table,embedd_class=class_table,embedd_punc=punc_table,
...
def in_domain_evaluation(args, datasets, model, optimizer, dev_eval_dict, test_eval_dict, epoch,
    ...
    curr_dev_eval_dict = evaluation(args, datasets['val'], 'val', model, args.domain, epoch, 'current_results')
    ...
    if epoch == args.num_epochs:
        ...
        for split in ['val', 'test']:
            eval_dict = dev_eval_dict['in_domain'] if split == 'val' else test_eval_dict['in_domain']
...
def evaluation(args, data, split, model, domain, epoch, str_res='results'):
    ...
    for batch in prepare_data.iterate_batch(data, args.batch_size, args.device):
        ...
        output, masks, lengths = model.forward(word, char, pos_type, case, number, gender, verb_form, full_pos, class_tag, punc_tag, mask=masks, length=lengths)
...
def write_results(args, data, data_domain, split, model, model_domain, eval_dict):
    ...
    for batch in prepare_data.iterate_batch(data, args.batch_size, args.device):        
        ...
        pred_writer.write(word, word_id,  word_pos_id, sen_id, pos_type, case, number, gender, verb_form, full_pos, heads, arc_tags, class_tag, punc_tag, lengths, auto_label=auto_label, symbolic_root=True)
        # writing gold labels
        gold_writer.write(word, word_id, word_pos_id, sen_id, pos_type, case, number, gender, verb_form, full_pos, heads, arc_tags, class_tag, punc_tag, lengths, symbolic_root=True, auto_label=auto_label)
...
def main():
    ...
    for split in args.splits:
        ...
        dataset = prepare_data.read_data_to_variable(args.data_paths[split], args.alphabets, args.device,
                                                     symbolic_root=True,use_aug=False)
