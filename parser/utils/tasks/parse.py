...
def eval_(words, pos_type_tag, heads_pred, arc_tag_pred, heads, arc_tag, word_alphabet, pos_type_alphabet, lengths,
    ...
    for i in range(batch_size):
        ...
        for j in range(start, lengths[i] - end):
            ...
            pos_type = pos_type_alphabet.get_instance(pos_type_tag[i, j])
            pos_type = pos_type.encode('utf8')
            ...
            if heads[i, j] == heads_pred[i, j]:
                ...
                ucm = 0
                lcm = 0
            if not is_punctuation(word, pos_type, punct_set):
                total_nopunc += 1
                if heads[i, j] == heads_pred[i, j]:
                    ucorr_nopunc += 1
                    if arc_tag[i, j] == arc_tag_pred[i, j]:
