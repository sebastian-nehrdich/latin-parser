...
class BiAAttention(nn.Module):
    ...
    def forward(self, input_d, input_e, mask_d=None, mask_e=None):
        ...
        out_d = torch.matmul(self.W_d, input_d.transpose(1, 2)).unsqueeze(3) # > b,lab,s,1 # TODO: Hier positional encodings einfuehren 
        ...
        out_e = torch.matmul(self.W_e, input_e.transpose(1, 2)).unsqueeze(2) # > b,lab,1,s
        ...
        if self.biaffine:
            ...
            output = output + out_d + out_e + self.b # TODO: Hier statt addition concat ueber torch.cat(dim=2)
        else:
            '''
            @bug: oh out_d + out_e instead?
            '''
            output = out_d + out_d + self.b # was geht hier genau vor sich? 
